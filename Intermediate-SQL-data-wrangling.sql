with base  as (
    SELECT
      events.title , 
      case when title ilike '%Hoadley%' then 'Hoadley'  
    ...
    case when title ilike '%Hoadley%' then 30
      ...
      case when creator__email_address ilike '%sunrisemovement.org%' then 'centralized' 
         when creator__email_address ilike '%kelsey.vizzard@gmail.com%' then 'centralized' 
         when creator__email_address ilike '%bb.holme@gmail.com%' then 'centralized' 
          when creator__email_address ilike '%mariana.snchez@gmail.com%' then 'centralized' 
...
by_event as (
 ...
 shift_goal - count(user__email_address) as remaining,
  ...
  from de_dup
  group by title_adj, start_date , shift_goal
  ) 
